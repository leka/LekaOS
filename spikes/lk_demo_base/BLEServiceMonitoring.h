...
class BLEServiceMonitoring : public interface::BLEService
{
	...
	const static uint16_t REBOOT_INSTRUCTION_WRITABLE_CHARACTERISTIC_UUID = 0x0001;
	const static uint16_t TURN_OFF_ALL_WRITABLE_CHARACTERISTIC_UUID		  = 0x0002;
	const static uint16_t PING_WRITABLE_CHARACTERISTIC_UUID				  = 0x0003;
	...
	const static uint8_t CHARACTERISTICS_COUNT = 8;
  ...
  public:
	...
	auto getTurnOffAll() const -> GattAttribute::Handle_t { return _enable_turn_off_characteristic_value; };
	...
	void onDataWritten(const GattWriteCallbackParams &params) final
	{
		...
		} else if (params.handle == _enable_turn_off_writable_characteristic.getValueHandle()) {
			std::fill_n(&_enable_turn_off_characteristic_value, 1, '\0');
			std::copy(params.data, params.data + 1, &_enable_turn_off_characteristic_value);
			_event_flags.set(BLE_TURN_OFF_ALL_FLAG);
  ...
  private:
	...
	bool _enable_turn_off_characteristic_value {};
	WriteOnlyGattCharacteristic<bool> _enable_turn_off_writable_characteristic {
		TURN_OFF_ALL_WRITABLE_CHARACTERISTIC_UUID, &_enable_turn_off_characteristic_value};

	uint8_t _enable_leds_characteristic_value {};
	WriteOnlyGattCharacteristic<uint8_t> _enable_leds_writable_characteristic {ENABLE_LEDS_WRITABLE_CHARACTERISTIC_UUID,
	...
	std::array<GattCharacteristic *, CHARACTERISTICS_COUNT> _characteristic_table {
		&_reboot_instruction_writable_characteristic,
		&_enable_turn_off_writable_characteristic,
		&_ping_writable_characteristic,
		&_enable_leds_writable_characteristic,
		&_enable_screen_writable_characteristic,
		&_enable_motors_writable_characteristic,
		&_leds_intensity_writable_characteristic,
