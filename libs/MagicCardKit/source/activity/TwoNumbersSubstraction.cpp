...
auto TwoNumbersSubstraction::playReinforcer() -> bool
{
	return _play_reinforcer;
}

void TwoNumbersSubstraction::setUtils(ReinforcerKit &reinforcerkit, interface::VideoKit &videokit, interface::LED &led)
{
...
void TwoNumbersSubstraction::registerCallback(const std::function<void(const MagicCard &card)> &callback)
{
	// do nothing
}

void TwoNumbersSubstraction::start()
{
	uint8_t pos					  = 0;
...
void TwoNumbersSubstraction::stop()
{
	// _behaviorkit->stop();
	_play_reinforcer = false;
}

void TwoNumbersSubstraction::run(const MagicCard &card)
{
...
void TwoNumbersSubstraction::setRandomNumberDisplay()
{
	do {
