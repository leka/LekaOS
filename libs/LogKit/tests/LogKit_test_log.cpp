...
TEST_F(LogKitTest, logDebug)
{
	...
	ASSERT_THAT(spy_trace_output, HasSubstr("[DBUG]"));
	ASSERT_THAT(spy_trace_output, HasSubstr("Hello, World"));
}
...
TEST_F(LogKitTest, logInfo)
{
	...
	ASSERT_THAT(spy_trace_output, HasSubstr("[INFO]"));
	ASSERT_THAT(spy_trace_output, HasSubstr("Hello, World"));
}
...
TEST_F(LogKitTest, logError)
{
	...
	ASSERT_THAT(spy_trace_output, HasSubstr("[ERR ]"));
	ASSERT_THAT(spy_trace_output, HasSubstr("Hello, World"));
}
...
TEST_F(LogKitTest, correctLineNumber)
{
	...
	ASSERT_THAT(spy_trace_output, MatchesRegex(".+ \\[.+[.h|.cpp]:" + line + "\\] .*"));
}
...
TEST_F(LogKitTest, variadicArguments)
{
	...
	ASSERT_THAT(spy_trace_output, MatchesRegex(".+ \\[INFO\\] \\[.*\\] .* > .* with 4 variadic arguments\n"));
}
