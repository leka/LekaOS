...
TEST_F(StateMachineTest, stateScreensaverCommandReceived)
{
...
TEST_F(StateMachineTest, stateSleepingEventBleConnection)
{
	...
	EXPECT_CALL(mock_rc, startConnectionBehavior(true)).Times(1);
...
TEST_F(StateMachineTest, stateScreensaverEventBleConnection)
{
	sm.set_current_states(lksm::state::screensaver, lksm::state::disconnected);
...
TEST_F(StateMachineTest, stateChargingEventBleConnection)
{
	...
	EXPECT_CALL(mock_rc, startChargingBehavior).Times(1);
	EXPECT_CALL(mock_rc, isCharging).WillRepeatedly(Return(true));
	EXPECT_CALL(mock_rc, startConnectionBehavior(false)).Times(1);
