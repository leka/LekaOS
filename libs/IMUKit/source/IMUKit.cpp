...
void IMUKit::init()
{
	...
	auto on_data_available_callback = [this](const interface::IMU::SensorData &data) { data_available_callback(data); };

	_imu.registerOnDataAvailableCallback(on_data_available_callback);
}
...
void IMUKit::data_available_callback(const interface::IMU::SensorData data)
{
