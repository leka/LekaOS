...
auto FirmwareKit::getCurrentVersionFromFile() -> FirmwareVersion
{
	...
	if (auto is_not_open = !_file.open(_os_version_path); is_not_open) {
...
auto FirmwareKit::loadUpdate(const FirmwareVersion &version) -> bool
{
	...
	snprintf(path.data(), std::size(path), _firmware_bin_path_format, version.major, version.minor, version.revision);
