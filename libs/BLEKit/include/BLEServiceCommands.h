...
{
  public:
	explicit BLEServiceCommands(CommandKit &kit)
		: interface::BLEService(service::commands::uuid, _characteristic_table), _cmdkit(kit) {};
	...
	void onCommandsReceived(std::function<void(std::span<uint8_t>)> callback) { _on_commands_received = callback; }
