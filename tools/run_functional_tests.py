...
TESTS_FUNCTIONAL_ROOT_DIRECTORY = "_build/LEKA_V1_2_DEV/tests/functional/sources/"
...
class Test:
    ...
    def flash(self):
        ...
        sleep(1)
        CMD_RESET = "openocd -f interface/stlink.cfg -c 'transport select hla_swd' -f target/stm32f7x.cfg -c init -c 'reset run' -c exit"
...
def main():
    ...
    reboot_device()
    remains = ""
    while remains is not None:
        remains = wait_for_response()
