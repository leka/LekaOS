...
class LKCoreSTM32HalMock : public LKCoreSTM32HalBase
{
  public:
	...
	MOCK_METHOD(void, HAL_LINKDMA, (SDRAM_HandleTypeDef * hsdram, DMA_HandleTypeDef *hdma, DMA_HandleTypeDef dma_handle), (override));
	...
	MOCK_METHOD(void, HAL_NVIC_SetPriority, (IRQn_Type IRQn, uint32_t PreemptPriority, uint32_t SubPriority), (override));
	MOCK_METHOD(void, HAL_NVIC_EnableIRQ, (IRQn_Type IRQn), (override));

	MOCK_METHOD(HAL_StatusTypeDef, HAL_SDRAM_Init, (SDRAM_HandleTypeDef * hsdram, FMC_SDRAM_TimingTypeDef *Timing), (override));
	...
	MOCK_METHOD(HAL_StatusTypeDef, HAL_SDRAM_ProgramRefreshRate, (SDRAM_HandleTypeDef * hsdram, uint32_t RefreshRate), (override));
	...
	MOCK_METHOD(HAL_StatusTypeDef, HAL_DMA2D_ConfigLayer, (DMA2D_HandleTypeDef * hdma2d, uint32_t LayerIdx), (override));
	MOCK_METHOD(HAL_StatusTypeDef, HAL_DMA2D_Start,
				(DMA2D_HandleTypeDef * hdma2d, uint32_t pdata, uint32_t DstAddress, uint32_t Width, uint32_t Height), (override));
	MOCK_METHOD(HAL_StatusTypeDef, HAL_DMA2D_PollForTransfer, (DMA2D_HandleTypeDef * hdma2d, uint32_t Timeout), (override));
	...
	MOCK_METHOD(HAL_StatusTypeDef, HAL_DSI_ConfigVideoMode, (DSI_HandleTypeDef * hdsi, DSI_VidCfgTypeDef *VidCfg), (override));
	...
	MOCK_METHOD(HAL_StatusTypeDef, HAL_DSI_ShortWrite,
				(DSI_HandleTypeDef * hdsi, uint32_t ChannelID, uint32_t Mode, uint32_t Param1, uint32_t Param2), (override));
	MOCK_METHOD(HAL_StatusTypeDef, HAL_DSI_LongWrite,
				(DSI_HandleTypeDef * hdsi, uint32_t ChannelID, uint32_t Mode, uint32_t NbParams, uint32_t Param1, uint8_t *ParametersTable),
				(override));

	MOCK_METHOD(HAL_StatusTypeDef, HAL_LTDC_StructInitFromVideoConfig, (LTDC_HandleTypeDef * hltdc, DSI_VidCfgTypeDef *VidCfg), (override));
	MOCK_METHOD(HAL_StatusTypeDef, HAL_LTDC_Init, (LTDC_HandleTypeDef * hltdc), (override));
	MOCK_METHOD(HAL_StatusTypeDef, HAL_LTDC_ConfigLayer, (LTDC_HandleTypeDef * hltdc, LTDC_LayerCfgTypeDef *pLayerCfg, uint32_t LayerIdx),
				(override));
	...
	MOCK_METHOD(HAL_StatusTypeDef, HAL_JPEG_Decode,
				(JPEG_HandleTypeDef * hjpeg, uint8_t *pDataIn, uint32_t InDataLength, uint8_t *pDataOutMCU, uint32_t OutDataLength,
				 uint32_t Timeout),
				(override));
	MOCK_METHOD(void, HAL_JPEG_ConfigInputBuffer, (JPEG_HandleTypeDef * hjpeg, uint8_t *pNewInputBuffer, uint32_t InDataLength),
				(override));
	MOCK_METHOD(void, HAL_JPEG_ConfigOutputBuffer, (JPEG_HandleTypeDef * hjpeg, uint8_t *pNewOutputBuffer, uint32_t OutDataLength),
				(override));
};
