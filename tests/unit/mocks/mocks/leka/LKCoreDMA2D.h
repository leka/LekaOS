...
class LKCoreDMA2DMock : public LKCoreDMA2DBase
{
  public:
	...
	MOCK_METHOD(void, transferData, (uintptr_t input_data_address, uintptr_t output_data_address, uint32_t width, uint32_t height),
				(override));
	MOCK_METHOD(void, transferImage, (uint32_t width, uint32_t height, uint32_t width_offset), (override));
	MOCK_METHOD(void, transferDrawing, (uintptr_t first_pixel_address, uint32_t width, uint32_t height, uint32_t color), (override));
