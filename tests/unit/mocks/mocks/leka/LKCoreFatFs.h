...
class LKCoreFatFsMock : public LKCoreFatFsBase
{
  public:
	...
	MOCK_METHOD(FRESULT, read, (void *buffer, uint32_t number_of_bytes_to_read, uint32_t *number_of_bytes_read), (override));
	MOCK_METHOD(FRESULT, write, (const void *buffer, uint32_t number_of_bytes_to_write, uint32_t *number_of_bytes_written), (override));
