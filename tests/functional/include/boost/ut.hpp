...
namespace detail {
	...
	struct cfg {
		using value_ref = std::variant<std::monostate, std::reference_wrapper<bool>,
									   std::reference_wrapper<std::size_t>, std::reference_wrapper<std::string>>;
...
class reporter_junit
{
  ...
  protected:
	...
	void print_result(std::ostream &stream, const std::string &suite_name, const std::string &indent,
...
namespace detail {
	...
	struct test {
		...
		constexpr void operator=(void (*_test)(std::string_view, std::string_view)) const { _test(type, name); }
