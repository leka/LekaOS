...
suite suite_core_video = [] {
	scenario("video initialization") = [] {
		given("video is in default configuration") = [] {
			corevideo.initialize();

			corelcd.setBrightness(1.F);
	...
	scenario("video turnOn turnOff enableDeepSleep turnOff") = [] {
		given("video is in default configuration") = [] {
			when("I turn on video") = [&] {
				corelcd.turnOn();
