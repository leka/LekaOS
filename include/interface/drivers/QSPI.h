...
class QSPI
{
  public:
	...
	virtual auto read(uint8_t command, int alternate_phase, int address, lstd::span<char> rx_buffer) -> size_t = 0;
	virtual auto write(uint8_t command, int alternate_phase, int address, const lstd::span<char> tx_buffer)
		-> size_t = 0;

	virtual auto sendCommand(uint8_t command, int address, const lstd::span<char> tx_buffer, lstd::span<char> rx_buffer)
