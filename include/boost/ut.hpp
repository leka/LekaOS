  ...
  protected:
	void print_duration(auto &printer) const noexcept
	{
		if (detail::cfg::show_duration) {
			std::int64_t time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(active_scope_->run_stop -
																						 active_scope_->run_start)
									   .count();
		...
		for (const auto &[suite_name, suite_result]: results_) {
			if (suite_result.fails) {
				err_stream << "\n========================================================"
							  "=======================\n"
						   << "Suite " << suite_name << '\n'   //
						   << "tests:   " << (suite_result.n_tests) << " | " << color_.fail << suite_result.fails
						   << " failed" << color_.none << '\n'
	...
	void print_junit_summary(std::ostream &stream)
	{
		...
		auto suite_time = [](auto const &suite_result) {
			std::int64_t time_ms =
				std::chrono::duration_cast<std::chrono::milliseconds>(suite_result.run_stop - suite_result.run_start)
					.count();
	...
	auto on(events::test<Ts...> test)
	{
		...
		if (detail::cfg::list_tags) {
			std::for_each(test.tag.cbegin(), test.tag.cend(),
						  [](const auto &tag) { std::cout << "tag: " << tag << std::endl; });
			return;
		}
...
namespace detail {
	...
	struct test {
		...
		template <class Test,
				  type_traits::requires_t<not type_traits::is_convertible_v<Test, void (*)(std::string_view)>> = 0>
		...
		return [f, t](const auto name) {
			for (const auto &arg: t) {
				detail::on<F>(events::test<F, decltype(arg)> {
					.type = "test", .name = std::string {name}, .tag = {}, .location = {}, .arg = arg, .run = f});
			}
		};
	}
			...
			apply(
				[f, name](const auto &...args) {
					(detail::on<F>(events::test<F, Ts> {
						 .type = "test", .name = std::string {name}, .tag = {}, .location = {}, .arg = args, .run = f}),
					 ...);
				},
				t);
		};
...
template <class TExpr,
		  type_traits::requires_t<type_traits::is_op_v<TExpr> or type_traits::is_convertible_v<TExpr, bool>> = 0>
...
template <class TLhs, class TRhs,
		  typename = type_traits::requires_t<type_traits::is_stream_insertable_v<TLhs> and
