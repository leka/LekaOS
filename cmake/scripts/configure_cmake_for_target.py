...
try:
    ...
    print("Be sure that you have installed all required python modules, see ",
          mbed_os_dir, "/requirements.txt")
    traceback.print_exc()
    exit(1)
...
""")
    ...
    for definition in sorted(symbol_list):
        ...
        if with_value_match is not None:
            target_header.write(
                "#define " + with_value_match.group(1) + " " + with_value_match.group(2) + "\n")
        else:
...
""")
    ...
    cmake_config.write(build_cmake_list(
        "MBED_TOOLCHAIN_NAME", [toolchain_name]))

    # note: CMake convention is that variables called "FLAGS" are strings while variables called "OPTIONS" are lists.
    cmake_config.write(build_cmake_list(
        "MCU_COMPILE_OPTIONS", mcu_compile_flags))
    cmake_config.write(build_cmake_list("MCU_LINK_OPTIONS", mcu_link_flags))
    cmake_config.write(build_cmake_path_list(
        "MBED_LINKER_SCRIPT", [linker_script]))
    cmake_config.write(build_cmake_path_list(
        "MBED_INCLUDE_DIRS", include_dirs))
    cmake_config.write(build_cmake_path_list(
        "MBED_SOURCE_FILES", source_files))

    for profile_idx in range(0, len(profile_names)):
        cmake_config.write(build_cmake_list(
            "MCU_COMPILE_OPTIONS_" + profile_names[profile_idx], profile_flags[profile_idx]))

    for profile_idx in range(0, len(profile_names)):
        cmake_config.write(build_cmake_list(
            "MCU_LINK_OPTIONS_" + profile_names[profile_idx], profile_link_flags[profile_idx]))
...
parser = argparse.ArgumentParser(
    description="Configure mbed-cmake for a processor target.")

parser.add_argument('target', type=str,
                    help="Mbed target name to configure the build system for.")

parser.add_argument('-l', '--list-config', action="store_true",
                    help="List all the config options that Mbed OS supports for your target")
parser.add_argument('-c', '--print-config', action="store_true",
                    help="Print all the config options that Mbed OS supports for your target, with descriptions")
...
pathlib.Path(config_header_dir).mkdir(
...
for profile_json_path in profile_jsons:
    with open(profile_json_path) as profile_file:
        ...
        profile_toolchain = build_api.prepare_toolchain(src_paths=[
                                                        mbed_os_dir], build_dir=config_header_dir, target=target_name, toolchain_name=toolchain_name, build_profile=[profile_data], app_config=app_config_path)
...
pathlib.Path(cmake_config_dir).mkdir(
    parents=True, exist_ok=True)  # create dir if not exists
...
for profile_idx in range(1, len(profile_toolchains)):
    ...
    common_flags = common_flags & set(
        profile_toolchains[profile_idx].flags['common'])
    common_link_flags = common_link_flags & set(
        profile_toolchains[profile_idx].flags['ld'])
...
for profile_idx in range(0, len(profile_toolchains)):
    profile_flags.append(
        set(profile_toolchains[profile_idx].flags['common']) - common_flags)
    profile_link_flags.append(
        set(profile_toolchains[profile_idx].flags['ld']) - common_link_flags)

write_cmake_config(cmake_config_path,
                   mcu_compile_flags=toolchain.flags['common'],
                   mcu_link_flags=toolchain.flags['ld'],
                   linker_script=resources.get_file_paths(
