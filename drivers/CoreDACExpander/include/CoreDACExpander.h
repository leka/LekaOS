...
class CoreDACExpanderMCP4728
{
  public:
	...
	void sequential_write_for_dac_input_registers_and_eeprom(uint8_t address, uint8_t channel, uint16_t value_calib);

	void single_write_for_dac_input_register_and_eeprom(uint8_t address, uint8_t channel, uint16_t value_calib);

	void write_vref(uint8_t address, uint8_t vref);

	void write_power_down(uint8_t address, uint8_t power_down);

	void write_gain(uint8_t address, uint8_t gain);
