...
{
	auto buffer	 = std::array<uint8_t, 3> {};
	buffer.at(0) = static_cast<uint8_t>(0x58 + (channel << 1 & 0x06));
	buffer.at(1) = static_cast<uint8_t>((value_calib & 0x0F00) >> 8);
	buffer.at(2) = static_cast<uint8_t>((value_calib & 0x00FF) >> 0);
	_i2c.write(address, buffer.data(), 3, false);
}

void CoreDACExpanderMCP4728::set_vref(uint8_t address, uint8_t vref)
{
...
void CoreDACExpanderMCP4728::set_power_down(uint8_t address, uint8_t power_down)
{
...
void CoreDACExpanderMCP4728::set_gain(uint8_t address, uint8_t gain)
{
	auto buffer	 = std::array<uint8_t, 1> {};
