...
class CFastLED
{
  ...
  public:
	...
	/// @tparam DATA_PIN - the optional data pin for the leds (if omitted, will default to the first hardware SPI MOSI
	/// pin)
	/// @tparam CLOCK_PIN - the optional clock pin for the leds (if omitted, will default to the first hardware SPI
	/// clock pin)
	/// @tparam RGB_ORDER - the rgb ordering for the leds (e.g. what order red, green, and blue data is written out in)
	/// @tparam SPI_DATA_RATE - the data rate to drive the SPI clock at, defined using DATA_RATE_MHZ or DATA_RATE_KHZ
	/// macros
	/// @returns a reference to the added controller
	...
	static CLEDController &addLeds(struct CRGB *data, mbed::SPI &spi, int nLedsOrOffset, int nLedsIfOffset = 0)
	{
		switch (CHIPSET) {
			...
			// }
			case SK9822: {
				static SK9822Controller<DATA_PIN, CLOCK_PIN, RGB_ORDER> c(spi);
				return addLeds(&c, data, nLedsOrOffset, nLedsIfOffset);
			}
		}
	}
	...
	/// CRGB data begins, and the third argument is the number of leds for this controller object.
	///
	/// This method also takes a 2 to 3 template parameters for identifying the specific chipset, data pin, and rgb
	/// ordering RGB ordering, and SPI data rate
	/// @param data - base point to an array of CRGB data structures
	/// @param nLedsOrOffset - number of leds (3 argument version) or offset into the data array
	/// @param nLedsIfOffset - number of leds (4 argument version)
	...
	/// led data, and the number of leds used by this controller.  The second is with 3 arguments, in which case
	/// the first  argument is the same, the second argument is an offset into the CRGB data where this controller's
	/// CRGB data begins, and the third argument is the number of leds for this controller object. This class includes
	/// the SmartMatrix and OctoWS2811 based controllers
	///
