...
/// Base definition for an LED controller.  Pretty much the methods that every LED controller object will make
/// available. Note that the showARGB method is not impelemented for all controllers yet.   Note also the methods for
/// eventual checking of background writing of data (I'm looking at you, teensy 3.0 DMA controller!).  If you want to
/// pass LED controllers around to methods, make them references to this type, keeps your code saner.  However, most
...
class CLEDController
{
  ...
  public:
	...
	CLEDController()
		: m_Data(NULL),
		  m_ColorCorrection(UncorrectedColor),
		  m_ColorTemperature(UncorrectedTemperature),
		  m_DitherMode(BINARY_DITHER),
		  m_nLeds(0)
	{
		m_pNext = NULL;
		if (m_pHead == NULL) {
	...
	CRGB getAdjustment(uint8_t scale) { return computeAdjustment(scale, m_ColorCorrection, m_ColorTemperature); }
...
#define RECOMMENDED_VIRTUAL_BITS                                                                                       \
	((UPDATES_PER_FULL_DITHER_CYCLE > 1) + (UPDATES_PER_FULL_DITHER_CYCLE > 2) + (UPDATES_PER_FULL_DITHER_CYCLE > 4) + \
	 (UPDATES_PER_FULL_DITHER_CYCLE > 8) + (UPDATES_PER_FULL_DITHER_CYCLE > 16) +                                      \
	 (UPDATES_PER_FULL_DITHER_CYCLE > 32) + (UPDATES_PER_FULL_DITHER_CYCLE > 64) +                                     \
...
#define VIRTUAL_BITS RECOMMENDED_VIRTUAL_BITS
	...
	__attribute__((always_inline)) inline bool has(int n) { return mLenRemaining >= n; }
	...
	__attribute__((always_inline)) inline void preStepFirstByteDithering() { d[RO(0)] = e[RO(0)] - d[RO(0)]; }
	...
	template <int SLOT>
	__attribute__((always_inline)) inline static uint8_t dither(PixelController &pc, uint8_t b)
	{
		return b ? qadd8(b, pc.d[RO(SLOT)]) : 0;
	}
	template <int SLOT>
	__attribute__((always_inline)) inline static uint8_t dither(PixelController &, uint8_t b, uint8_t d)
	{
	...
	template <int SLOT>
	__attribute__((always_inline)) inline static uint8_t scale(PixelController &, uint8_t b, uint8_t scale)
	{
	...
	template <int SLOT>
	__attribute__((always_inline)) inline static uint8_t loadAndScale(PixelController &pc, int lane)
	{
	...
	template <int SLOT>
	__attribute__((always_inline)) inline static uint8_t loadAndScale(PixelController &pc, int lane, uint8_t scale)
	{
	...
	__attribute__((always_inline)) inline uint8_t stepAdvanceAndLoadAndScale0(int lane, uint8_t scale)
	{
		stepDithering();
		return advanceAndLoadAndScale<0>(*this, lane, scale);
	}
