...
class CoreHTS : public interface::TemperatureSensor, public interface::HumiditySensor
{
  ...
  private:
	...
	static auto ptr_io_write(void *handle, uint8_t register_address, uint8_t *p_buffer, uint16_t number_bytes_to_write) -> int32_t;
	static auto ptr_io_read(void *handle, uint8_t register_address, uint8_t *p_buffer, uint16_t number_bytes_to_read) -> int32_t;
