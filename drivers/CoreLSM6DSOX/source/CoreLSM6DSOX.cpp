...
namespace leka {
	...
	{
		return lsm6dsox_int_notification_get(ctx, val);
	}
	...
	{
		if(lsm6dsox_mem_bank_set(ctx, LSM6DSOX_EMBEDDED_FUNC_BANK))
			return 1;

		if (lsm6dsox_read_reg(ctx, LSM6DSOX_MLC0_SRC, buff, 8))
			return 1;

		if (lsm6dsox_mem_bank_set(ctx, LSM6DSOX_USER_BANK))
			return 1;
